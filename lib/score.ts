type Profile = {
  ...
  bio?: string | null
  cgpa?: number | null
  links?: { linkedin?: string | null; portfolio?: string | null } | null
  skills?: unknown[]
  projects?: unknown[]
...
export function breakdown(p: Profile, m: Record<string, number> = {}) {
  return {
    profile: cap([p.headline, p.bio, p.links?.linkedin, p.links?.portfolio, p.cgpa].filter(Boolean).length / 5, 10),
...
export const profileStrength = (p: Profile, m?: Record<string, number>) =>
